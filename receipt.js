...
class ReceiptModal {
                ...
                <div class="receipt-container">
                    ...
                    <div class="receipt-body">
                        ...
                        <div class="receipt-actions" id="receiptActions" style="display: none;">
                            ...
                            <button id="screenshotReceiptBtn" class="receipt-btn screenshot">
                                📸 Save Screenshot
                            </button>
    ...
    setupEventListeners() {
        ...
        document.getElementById('screenshotReceiptBtn')?.addEventListener('click', () => {
            this.takeScreenshot();
        });
    }
                ...
                detailsHTML += `
                    <div class="receipt-section">
                        ...
                        <div class="receipt-row">
                            ...
                            <strong>${data.startQuarter || '-'}</strong>
                        </div>
                        <div class="receipt-row">
                            <span>Lock Period:</span>
                            <strong>${data.lockPeriod || '180-day phases x 5'}</strong>
                        </div>
                    </div>
                `;
                break;
    ...
    // ────────────────────────────────────────────────────────────
    // SCREENSHOT: HD quality receipt screenshot using html2canvas
    // ────────────────────────────────────────────────────────────
    async takeScreenshot() {
        const btn = document.getElementById('screenshotReceiptBtn');
        ...
        try {
            ...
            // Hide UI elements that shouldn't appear in screenshot
            const actionsDiv = document.getElementById('receiptActions');
            const closeBtn = document.getElementById('closeReceipt');
            const overlay = modal?.querySelector('.receipt-overlay');
            ...
            // Hide dark overlay so background doesn't bleed into capture
            if (overlay) overlay.style.display = 'none';

            // Temporarily remove modal background so container renders clean
            const prevModalBg = modal.style.background;
            ...
            const canvas = await html2canvas(container, {
                ...
                ignoreElements: (el) => el.classList.contains('receipt-overlay')
            });
            ...
            modal.style.background = prevModalBg;

            // Download as PNG
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
            const filename = `EYSN-receipt-${timestamp}.png`;
            ...
            btn.disabled = false;
            setTimeout(() => btn.textContent = '📸 Save Screenshot', 2000);
        }
    }
